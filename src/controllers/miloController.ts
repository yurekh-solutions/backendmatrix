...
import MiloChat from '../models/MiloChat';
...
const generateMiloResponse = async (
  userMessage: string,
  context: any,
  userType: string
  ...
  let response = '';
  let metadata: Record<string, any> = { intent };
...
export const getOrCreateChat = async (req: Request, res: Response) => {
  try {
    ...
    let chat = await MiloChat.findOne({
      $or: [
        { userId, userType },
        { supplierId, userType },
      ],
    ...
    if (!chat) {
      ...
      const { response, metadata } = await generateMiloResponse(
        'Hello',
        chat.context,
        userType
      );
...
export const sendMessage = async (req: Request, res: Response) => {
  try {
    ...
    const chat = await MiloChat.findById(chatId);
    if (!chat) {
...
export const getChatHistory = async (req: Request, res: Response) => {
  try {
    ...
    const chats = await MiloChat.find(query)
      .sort({ createdAt: -1 })
      .limit(50);
...
export const rateChat = async (req: Request, res: Response) => {
  try {
    ...
    const chat = await MiloChat.findByIdAndUpdate(
      chatId,
      {
        helpful,
        summary: feedback,
  ...
  try {
    ...
    res.json({
      ...
      analytics: {
        ...
        topTopics,
        avgMessagesPerChat: (
          chats.reduce((sum, c) => sum + c.messages.length, 0) / totalChats
        ).toFixed(1),
      },
    });
  } catch (error: any) {
