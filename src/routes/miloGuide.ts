...
router.get('/guide/buyer', async (req: any, res) => {
  try {
    // Get hot/popular products
    const rfqs = await RFQ.find().select('productCategory').limit(300);
    ...
    // Get all categories
    const categories = await Category.find({}).select('name slug icon').limit(20);

    // Get top suppliers
    const topSuppliers = await Supplier.find({ status: 'approved' })
      ...
      sections: {
        ...
        categories: {
          ...
          list: categories.slice(0, 10).map(cat => ({
            ...
            topSuppliers: `${topSuppliers.filter(s => 
              s.productsOffered.some(p => p.toLowerCase().includes(cat.name.toLowerCase()))
            ).length}+ suppliers`
          }))
        },
        supplierInfo: {
...
router.get('/guide/supplier', async (req: any, res) => {
  try {
    ...
    const totalSuppliers = await Supplier.countDocuments({ status: 'approved' });
    const recentSuppliers = await Supplier.find({ status: 'approved' })
...
router.get('/guide/product/:category', async (req: any, res) => {
  try {
    ...
    const rfqs = await RFQ.find({ productCategory: category })
    ...
    const productGuide = {
      ...
      topSuppliers: suppliers.slice(0, 5).map(s => ({
        ...
        phone: s.phone,
        experience: `${s.yearsInBusiness} years`,
        productsInCategory: s.productsOffered.filter(p => 
          p.toLowerCase().includes(category.toLowerCase())
        )
      })),
      sampleProducts: products.slice(0, 5).map(p => ({
        name: p.name,
        supplier: typeof p.supplierId === 'object' ? (p.supplierId as any).companyName : 'Unknown',
...
// Get RFQ guide
router.get('/guide/rfq', async (req: any, res) => {
  try {
    const rfqGuide = {
...
router.get('/guide/automation', async (req: any, res) => {
  try {
...
// Get WhatsApp guide
router.get('/guide/whatsapp', async (req: any, res) => {
  try {
    const whatsappGuide = {
...
router.get('/guide/navigation', async (req: any, res) => {
  try {
...
router.get('/guide/complete', async (req: any, res) => {
  try {
