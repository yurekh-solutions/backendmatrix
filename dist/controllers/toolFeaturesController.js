...
exports.getToolFeatures = exports.getBusinessAnalytics = exports.optimizePrice = exports.updateInventory = exports.automateOrder = exports.scoreLead = exports.createAutoReplyTemplate = void 0;
const Product_1 = __importDefault(require("../models/Product"));
const ToolUsage_1 = __importDefault(require("../models/ToolUsage"));
const createAutoReplyTemplate = async (req, res) => {
    try {
        ...
        // Update tool usage
        await ToolUsage_1.default.findOneAndUpdate({ supplierId, toolType: 'auto-reply' }, {
            status: 'active',
            lastUsedAt: new Date(),
            $inc: { usageCount: 1 }
        });
...
exports.createAutoReplyTemplate = createAutoReplyTemplate;
// ========== 2. LEAD SCORING ==========
/**
...
const scoreLead = async (req, res) => {
    try {
        ...
        if (score >= 80)
            ...
            quality = 'warm';
        // Update tool usage
        await ToolUsage_1.default.findOneAndUpdate({ supplierId, toolType: 'lead-scoring' }, {
            status: 'active',
            lastUsedAt: new Date(),
            $inc: { usageCount: 1 },
            metrics: {
...
const automateOrder = async (req, res) => {
    try {
        ...
        await ToolUsage_1.default.findOneAndUpdate({ supplierId, toolType: 'order-automation' }, {
            status: 'active',
            lastUsedAt: new Date(),
            $inc: { usageCount: 1 },
            metrics: {
...
const updateInventory = async (req, res) => {
    try {
        ...
        const supplierId = req.supplier?._id;
        const product = await Product_1.default.findOne({ _id: productId, supplierId });
        if (!product) {
        ...
        await product.save();
        // Update tool usage
        await ToolUsage_1.default.findOneAndUpdate({ supplierId, toolType: 'smart-inventory' }, {
            status: 'active',
            lastUsedAt: new Date(),
            $inc: { usageCount: 1 },
            metrics: {
...
const optimizePrice = async (req, res) => {
    try {
        ...
        // Update tool usage
        await ToolUsage_1.default.findOneAndUpdate({ supplierId, toolType: 'price-optimizer' }, {
            status: 'active',
            lastUsedAt: new Date(),
            $inc: { usageCount: 1 },
            metrics: {
...
const getBusinessAnalytics = async (req, res) => {
    try {
        ...
        const products = await Product_1.default.find({ supplierId, status: 'active' });
        // Get tool usage data
        const toolUsage = await ToolUsage_1.default.find({ supplierId });
        // Calculate metrics
        ...
        const performanceScore = Math.round((activeProducts / totalProducts * 30) + // 30% from active products
        ...
        // Update tool usage
        await ToolUsage_1.default.findOneAndUpdate({ supplierId, toolType: 'analytics-hub' }, {
            status: 'active',
            lastUsedAt: new Date(),
            $inc: { usageCount: 1 }
        });
 ...
 */
const getToolFeatures = async (req, res) => {
    try {
        const supplierId = req.supplier?._id;
        const toolUsage = await ToolUsage_1.default.find({ supplierId });
        const features = {
...
exports.getToolFeatures = getToolFeatures;
