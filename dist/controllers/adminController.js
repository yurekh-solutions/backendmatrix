...
const rejectProduct = async (req, res) => {
    try {
        ...
        const product = await Product_1.default.findById(req.params.id).populate('supplierId', 'email companyName');
        if (!product) {
            return res.status(404).json({ success: false, message: 'Product not found' });
        }
        ...
        `;
                await (0, email_1.sendEmail)(supplier.email, `Product Rejected: ${product.name}`, emailHtml);
            }
        }
        catch (emailError) {
